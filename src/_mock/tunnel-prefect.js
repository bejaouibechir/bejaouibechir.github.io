...
'def extract():\n'+
'    block = SqlAlchemyConnector.load("prod-mysql")\n'+
'    with block.get_connection() as conn:\n'+
...
'    logger.info("rolled up %s regions", len(df))\n'+
'    return df\n'+
'\n'+
...
'    daily_orders.serve(name="prod", cron="0 2 * * *")\n'},
...
'    engine = SqlAlchemyConnector.load("dwh").get_engine()\n'+
'    df.to_sql("signups", engine, if_exists="append", index=False)\n'+
'\n'+
'@flow(name="load_signups", log_prints=True)\n'+
'def load_signups():\n'+
'    load()\n'+
'\n'+
'if __name__ == "__main__":\n'+
'    load_signups.serve(name="hourly", cron="0 * * * *")\n'},
...
'@task\n'+
'def pull():\n'+
'    key = Secret.load("shop-api-key").get()\n'+
'    r = httpx.get("https://api.shop.io/v2/products", headers={"x-key": key})\n'+
...
'\n'+
'if __name__ == "__main__":\n'+
'    sync_products.serve(interval=timedelta(hours=6))\n'},
...
'\n'+
'@flow(name="clean_customers")\n'+
'def clean_customers():\n'+
'    clean()\n'+
'\n'+
'clean_customers.serve(name="weekly", cron="0 6 * * 1")\n'},
{n:'submit() / result() everywhere',f:'concurrent_etl.py',s:
'from prefect import flow, task\n'+
'import pandas as pd\n'+
'\n'+
'@task\n'+
'def read():\n'+
'    return pd.read_parquet("/lake/invoices.parquet")\n'+
'\n'+
'@task\n'+
'def shape(df):\n'+
'    df = df.query("status == \'paid\'")\n'+
'    df = df.groupby(["day", "country"]).agg({"total": "sum"}).reset_index()\n'+
'    return df\n'+
'\n'+
'@flow(name="concurrent_etl")\n'+
'def concurrent_etl():\n'+
'    fut = read.submit()\n'+
'    df = shape.submit(fut.result())\n'+
'    df.result().to_parquet("/marts/invoices.parquet")\n'+
'\n'+
'concurrent_etl.serve(cron="30 3 * * *")\n'},
{n:'map() over a list of files',f:'mapped_ingest.py',s:
'from prefect import flow, task\n'+
...
'\n'+
'@flow(name="mapped_ingest")\n'+
'def mapped_ingest():\n'+
'    files = list(Path("/drop").glob("*.csv"))\n'+
'    ingest.map(files)\n'+
'\n'+
'mapped_ingest.serve(cron="*/15 * * * *")\n'},
{n:'Dask task runner',f:'dask_rollup.py',s:
'from prefect import flow, task\n'+
...
'@task\n'+
'def rollup():\n'+
'    df = pd.read_parquet("/lake/events.parquet")\n'+
'    df = df[df.kind != "debug"]\n'+
'    df = df.groupby(["day", "kind"]).agg({"n": "sum"}).reset_index()\n'+
'    df.to_parquet("/marts/events_daily.parquet")\n'+
'\n'+
'@flow(name="dask_rollup", task_runner=DaskTaskRunner(cluster_kwargs={"n_workers": 8}))\n'+
'def dask_rollup():\n'+
'    rollup()\n'+
'\n'+
'dask_rollup.serve(cron="0 4 * * *")\n'},
{n:'Cached extract',f:'cached_extract.py',s:
'from prefect import flow, task\n'+
...
'\n'+
'@flow(name="cached_extract")\n'+
'def cached_extract():\n'+
'    fetch()\n'+
'\n'+
'cached_extract.serve(cron="0 */4 * * *")\n'},
{n:'Prefect 1.x legacy flow',f:'legacy_flow.py',s:
...
'\n'+
'with Flow("legacy_flow") as flow:\n'+
'    path = Parameter("path", default="/data/in.csv")\n'+
'    extract(path)\n'+
'\n'+
'flow.run()\n'},
{n:'Async flow',f:'async_sync.py',s:
'from prefect import flow, task\n'+
'import pandas as pd, httpx\n'+
'\n'+
'@task\n'+
...
'        r = await c.get("https://api.acme.io/v1/tickets")\n'+
'    df = pd.json_normalize(r.json())\n'+
'    df = df[df.priority >= 2]\n'+
'    df.to_parquet("/lake/tickets.parquet")\n'+
'\n'+
'@flow(name="async_sync")\n'+
'async def async_sync():\n'+
'    await pull()\n'+
'\n'+
'async_sync.serve(cron="*/30 * * * *")\n'}];
...
function secs(n){n=+n;if(n%3600===0)return (n/3600)+'h';if(n%60===0)return (n/60)+'m';return n+'s'}
function translate(text){
  var L=text.split('\n'),N=[],tf=[],srcs=[],dsts=[],acts=[],name=null,cron=null,manual=false,
      retries=null,backoff=null,cache=null,lName=null,lCron=null,lRetry=null,lCache=null;
  ...
  if(/^\s*deployments:/m.test(text)||/^\s*work_pool:/m.test(text))
    note('care',1,'prefect.yaml','This is a deployment manifest, not the flow. Paste the flow module — the data logic is what translates.');

  var blk=(/SqlAlchemyConnector\.load\(\s*["']([\w.\- ]+)["']/.exec(text)||[])[1]||null;
  var hookPg=/postgres|psql/i.test(blk||'')||/postgres/i.test(text),hookMy=/mysql|maria/i.test(blk||'');

  /* --- nom du flow --- */
  var r=first(/@flow\([^)]*name\s*=\s*["']([\w.\- ]+)["']/)||first(/with\s+Flow\(\s*["']([\w.\- ]+)["']/);
  if(!r){for(var i=0;i<L.length;i++){if(/^\s*@flow\b/.test(L[i])){
    for(var j=i+1;j<Math.min(L.length,i+4);j++){var dm=/^\s*(?:async\s+)?def\s+(\w+)\s*\(/.exec(L[j]);
      if(dm){r={m:[0,dm[1]],l:j+1};break}}
    if(r)break}}}
  if(r){name=r.m[1];lName=r.l;note('map',r.l,'flow name','name: '+name)}

  /* --- planification : serve / deploy / schedules --- */
  r=first(/(?:serve|deploy)\([^)]*cron\s*=\s*["']([^"']+)["']/)||first(/CronSchedule\([^)]*cron\s*=\s*["']([^"']+)["']/)||first(/^\s*cron:\s*["']?([^"'\n]+)["']?/);
  if(r){cron=r.m[1].trim();lCron=r.l;note('map',r.l,'cron schedule','trigger.cron: '+q(cron))}
  else{r=first(/(?:serve|deploy)\([^)]*interval\s*=\s*timedelta\(([^)]*)\)/)||first(/IntervalSchedule\([^)]*timedelta\(([^)]*)\)/);
    if(r){cron=delta(r.m[1]);lCron=r.l;note('map',r.l,'interval schedule','trigger.cron: '+q(cron))}
    else{r=first(/\.(?:serve|deploy)\(/);
      if(r){manual=true;lCron=r.l;note('care',r.l,'no schedule on the deployment','The cron lives in your prefect.yaml or in Prefect Cloud, not in this file. Fill trigger.cron in by hand.')}}}

  /* --- reprises --- */
  r=first(/retries\s*=\s*(\d+)/);
  if(r){retries=+r.m[1];lRetry=r.l;
    var rd=/retry_delay_seconds\s*=\s*(\d+)/.exec(L[r.l-1])||/retry_delay_seconds\s*=\s*(\d+)/.exec(text);
    if(rd)backoff=secs(rd[1]);
    note('map',r.l,'retries','retry.attempts: '+retries+(backoff?', backoff: '+backoff:''))}

  /* --- cache --- */
  r=first(/cache_key_fn\s*=/);
  if(r){lCache=r.l;cache={ttl:null};
    var ce=/cache_expiration\s*=\s*timedelta\(([^)]*)\)/.exec(L[r.l-1])||/cache_expiration\s*=\s*timedelta\(([^)]*)\)/.exec(text);
    if(ce){var hh=/hours\s*=\s*(\d+)/.exec(ce[1]),mm=/minutes\s*=\s*(\d+)/.exec(ce[1]),dd=/days\s*=\s*(\d+)/.exec(ce[1]);
      cache.ttl=hh?hh[1]+'h':(mm?mm[1]+'m':(dd?(dd[1]*24)+'h':null))}
    note('map',r.l,'cache_key_fn','cache.enabled: true'+(cache.ttl?', ttl: '+cache.ttl:''))}

  /* --- noms des taches, pour reperer les boucles qui les appellent --- */
  var tasks={};
  for(var i=0;i<L.length;i++)if(/^\s*@task\b/.test(L[i]))
    for(var j=i+1;j<Math.min(L.length,i+4);j++){var d2=/^\s*(?:async\s+)?def\s+(\w+)\s*\(/.exec(L[j]);if(d2){tasks[d2[1]]=1;break}}
  var taskRe=Object.keys(tasks).length?new RegExp('\\b('+Object.keys(tasks).join('|')+')\\s*\\('):null;

  var imps=[],deco=[],logs=[],fut=[],asy=[];
  for(var i=0;i<L.length;i++){
    var ln=L[i],no=i+1,m;
    if(/^\s*(from|import)\s+/.test(ln)){imps.push(no);continue}
    if(/(Dask|Ray)TaskRunner/.test(ln)){note('care',no,'distributed task runner','Hydra v1 runs a job in one process. Distribution is planned through the mosaic node model, not a task runner.');continue}
    if(/^\s*@(task|flow)\b/.test(ln)){deco.push(no);continue}
    if(/get_run_logger\(|^\s*logger\.|print\(/.test(ln)){logs.push(no);continue}
    if(/^\s*if\s+__name__/.test(ln)){note('drop',no,'__main__ guard','A manifest has no entry point to guard.');continue}

    if(m=/(\w+)\.map\(/.exec(ln)){note('care',no,'.map()','Fan-out over a list is a workflow concern, not a job step. It becomes a loop node calling the same job N times.');continue}
    if(/\.submit\(|\.result\(\s*\)|PrefectFuture/.test(ln)){fut.push(no);
      ln=ln.replace(/\.result\(\s*\)/g,'').replace(/\.submit\(/g,'(');
      if(/^\s*\w+\s*=\s*\w+\s*\(\s*\w*\s*\)\s*$/.test(ln))continue}
    if(/^\s*async\s+(def|with|for)|await\s+/.test(ln)){asy.push(no);ln=ln.replace(/\bawait\s+/g,'');
      if(/^\s*async\s+(def|with|for)/.test(ln))continue}
    if(/run_deployment\(/.test(ln)){note('care',no,'run_deployment()','Calling another flow becomes a workflow step of type: job with depends_on.');continue}
    if(/Parameter\(/.test(ln)){note('care',no,'Parameter()','Prefect 1 construct. In Hydra a runtime value comes from ${env.X} or --set on the CLI.');continue}
    if(/^\s*flow\.run\(\)/.test(ln)){note('drop',no,'flow.run()','Removed in Prefect 2, and a manifest is run by hdrctl anyway.');continue}
    if(/create_(markdown|table|link)_artifact\(/.test(ln)){note('drop',no,'artifact','Run reporting is the runner’s job, not the manifest’s.');continue}
    if(/persist_result|result_storage/.test(ln)){note('care',no,'result persistence','Hydra persists what a destination declares, nothing more. Decide whether this intermediate result is really a destination.');continue}
    if(/\.with_options\(/.test(ln)){note('care',no,'with_options()','Runtime reconfiguration has no declarative equivalent — split into two manifests or use ${env.X}.');continue}
    if(/\.(serve|deploy)\(/.test(ln)){note('drop',no,'.serve() / .deploy()','Serving is the scheduler’s business. The cron above is all Hydra needs.');continue}
    if(m=/Secret\.load\(\s*["']([\w.\- ]+)["']/.exec(ln)){note('map',no,'Secret block','Resolved as '+secretName(m[1])+' from your secret store.');continue}
    if(m=/SqlAlchemyConnector\.load\(\s*["']([\w.\- ]+)["']/.exec(ln)){note('map',no,'SqlAlchemy block','source.conn: '+secretName(m[1]));continue}
    if(/(S3Bucket|GcsBucket|AzureBlobStorage)\.load\(/.test(ln)){note('map',no,'storage block','Becomes the path prefix plus a credential reference on the source or destination.');continue}
    if(taskRe&&/^\s*(for|while)\s/.test(ln)){
      var body=L.slice(i+1,i+4).join('\n');
      if(taskRe.test(body)){note('care',no,'loop calling a task','Dynamic fan-out. A job is a straight line — this becomes a workflow loop node.');continue}}

    /* ---- sources ---- */
    if(m=/pd\.read_sql\(\s*f?["']([^"']+)["']/.exec(ln)){srcs.push({t:hookPg?'postgres':(hookMy?'mysql':'sql'),q:m[1],l:no,conn:blk?secretName(blk):null});
    ...
    if(m=/(?:httpx|requests|client|c)\.(?:get|post)\(\s*["']([^"']+)["']/.exec(ln)){srcs.push({t:'webapi',p:m[1],l:no});note('map',no,'HTTP call','source.type: webapi');continue}
    ...
    if(m=/(?:pd\.read_(\w+)|\.to_(\w+))\(\s*(?!["'])[\w(]/.exec(ln)){
      note('care',no,'computed path','The path is built at runtime. A job declares one path — use ${run.date} in it, or make the fan-out a workflow loop.');continue}
  ...
  if(deco.length)note('drop',deco[0],deco.length+' decorator'+(deco.length>1?'s':''),'@flow and @task are wiring. The manifest declares steps; nothing needs decorating.');
  ...
  if(asy.length)note('care',asy[0],asy.length+' async construct'+(asy.length>1?'s':''),'Hydra steps are synchronous. Async concurrency inside a task has no declarative form — keep that part in Prefect, or wrap it in a plugin.');
  if(fut.length)note('drop',fut[0],fut.length+' submit/result line'+(fut.length>1?'s':''),'Prefect already hands data over in memory. Inside a job, steps are sequential, so futures have nothing left to coordinate.');
  ...
  return {name:name||'untitled',cron:cron,manual:manual,tf:tf,src:srcs[0]||null,dst:dsts[0]||null,acts:acts,
          retries:retries,backoff:backoff,cache:cache,notes:N,lName:lName,lCron:lCron,lRetry:lRetry,lCache:lCache,
          srcs:srcs,dsts:dsts,grp:{imp:imps,xcom:fut.concat(deco,logs),async:asy}};
}
...
function run(){
  var text=$("src").value;
  if(!text.trim()){$("yml").innerHTML='<span class="yc"># paste a Prefect flow on the left —\n# or pick a sample above.</span>';$("yml").dataset.raw='';
    $("fnB").textContent='job.yaml';$("cMap").textContent='0';$("cDrop").textContent='0';$("cCare").textContent='0';
    $("gut").innerHTML='<i>1</i>';$("rl").innerHTML='<li class="empty">Waiting for a flow.</li>';CUR=null;ROWS=[];return}
  ...
  $("fnA").textContent=(t.name==='untitled'?'flow':t.name)+'.py'+(ignCount()?'  ('+ignCount()+' ignored)':'');
...
var deb;
$("src").addEventListener('input',function(){$("ex").selectedIndex=-1;$("exTag").textContent='your flow';clearTimeout(deb);deb=setTimeout(run,120)});
...
$("blank").onclick=function(){$("src").value='';$("fnA").textContent='flow.py';$("exTag").textContent='';run();$("src").focus()};
load(0);
/* ================= MENU CONTEXTUEL ================= */
...
function reverse(block){
  ...
  if(block==='source'){var s=t.src;if(!s)return '# no source detected';
    ...
    return 'with SqlAlchemyConnector.load("dwh").get_connection() as conn:\n    df = pd.read_sql('+JSON.stringify(s.q||'')+', conn)'}
  ...
  if(block==='trigger'||block==='name'){var fn=String(t.name).replace(/[^\w]/g,'_');
    return '@flow(name='+JSON.stringify(t.name)+')\ndef '+fn+'():\n    ...\n\n'+fn+'.serve('+(t.cron?'cron='+JSON.stringify(t.cron):'name="manual"')+')'}
  if(block==='retry')return '@task(retries='+t.retries+(t.backoff?', retry_delay_seconds='+(parseInt(t.backoff,10)*(/h$/.test(t.backoff)?3600:(/m$/.test(t.backoff)?60:1))):'')+')';
  if(block==='cache')return '@task(cache_key_fn=task_input_hash'+(t.cache&&t.cache.ttl?', cache_expiration=timedelta(hours='+parseInt(t.cache.ttl,10)+')':'')+')';
  return '# '+block+' has no Prefect counterpart';
}
...
ctx($("src"),function(e){
  ...
  var items=[
    ...
    '-',
    {t:'Paste a flow',k:'⌘V',f:function(){
      if(navigator.clipboard&&navigator.clipboard.readText)
        navigator.clipboard.readText().then(function(txt){if(!txt)return;IGN={};SEL=null;ta.value=txt;$("ex").selectedIndex=-1;$("exTag").textContent='your flow';run();toast('Pasted')})
...
ctx($("yml"),function(e){
  ...
  return {head:b?b.name+':':'yaml',items:[
    ...
    {t:'Reveal the Prefect line',off:!src,f:function(){gotoLine(src);toast('Line '+src)}},
    '-',
    {t:'Back to Prefect (this block)',off:!b,f:function(){
      pop('reverse',b.name+': → Python','<p>The same block, expressed back as Prefect. Nothing here is a one-way door.</p><pre>'+esc(reverse(b.name))+'</pre>',e.clientX,e.clientY)}},
...
function loadText(txt,label){IGN={};MUTE={};SEL=null;$("src").value=txt;
  $("ex").selectedIndex=-1;$("exTag").textContent=label||'your flow';run();
...
$("icRev").onclick=function(){
  ...
  if(CUR.cache)out.unshift(reverse('cache'),'');
  if(CUR.retries!=null)out.unshift(reverse('retry'),'');
  var r=this.getBoundingClientRect();
  pop('reverse',CUR.name+' → Prefect','<p>The whole manifest, back as a flow. Round-tripping is not a one-way door.</p><pre>'+esc(out.join('\n'))+'</pre>',r.left-340,r.bottom+8)};
...
function lintPy(text){
  ...
  /* --- 4. copier-coller et pieges de version Prefect --- */
  info.forEach(function(x){
    ...
    if(/from\s+prefect\s+import\s+[^\n]*\bFlow\b|with\s+Flow\s*\(/.test(x.code))add('err',no,'Prefect 1.x Flow','The imperative Flow context was removed in Prefect 2. Rewrite as @flow before anything else.');
    if(/\bParameter\s*\(/.test(x.code))add('err',no,'Parameter()','Prefect 1 only. In Prefect 2+ a flow argument is an ordinary function parameter.');
    if(/\bflow\.run\s*\(/.test(x.code))add('err',no,'flow.run()','Removed in Prefect 2 — call the flow function directly.');
    if(/Deployment\.build_from_flow/.test(x.code))add('warn',no,'Deployment.build_from_flow','Removed in Prefect 3. Use flow.deploy() or prefect.yaml.');
    if(/prefect\.tasks\.shell|prefect\.engine|prefect\.utilities\.tasks/.test(x.code))add('warn',no,'Prefect 1.x module','This import path no longer exists in Prefect 2+.');
    if(/prefect\s+agent\b/.test(ln))add('info',no,'prefect agent','Agents were replaced by workers and work pools in Prefect 2.3.');
    if(/retry_delay_seconds/.test(x.code)&&!/retries\s*=/.test(text))add('warn',no,'delay without retries','retry_delay_seconds does nothing unless retries is set.');
    if(/cache_key_fn/.test(x.code)&&!/cache_expiration/.test(text))add('info',no,'cache never expires','No cache_expiration — the cached result is reused indefinitely.');
    if(/(Dask|Ray)TaskRunner/.test(x.code)&&!/prefect_(dask|ray)/.test(text))add('warn',no,'missing runner extra','The task runner is used but prefect-dask / prefect-ray is not imported.');
    if(!x.cont&&x.ind===0&&/\.(serve|deploy)\s*\(/.test(x.code)&&!/if\s+__name__/.test(text))
      add('info',no,'serve() at import time','Without an if __name__ == "__main__" guard, importing this module starts serving the flow.');
  });
  if(/@task\b/.test(text)&&!/@flow\b/.test(text)&&!/with\s+Flow\s*\(/.test(text))
    add('warn',1,'task without a flow','A @task can only run inside a flow. Nothing here will execute on its own.');
  if(/\.submit\s*\(/.test(text)&&!/\.result\s*\(\s*\)/.test(text))
    add('warn',1,'future never resolved','.submit() returns a PrefectFuture, not a DataFrame. Without .result() the next call receives the future itself.');
  if(!/@flow\b|with\s+Flow\s*\(/.test(text)){
    out=out.filter(function(x){return x.w!=='top-level I/O'});
    add('info',1,'no flow found','Parsed as a plain pandas script — the trigger falls back to manual.');}
...
function lintYaml(rows,t){
  ...
  rows.forEach(function(r,i){
    ...
    if(/\{\{/.test(l))add('warn',i,'template left in place','Prefect variables are not evaluated by Hydra. Use ${run.date} or ${env.X}.');
...
$("icLint").onclick=function(){var it=lintPy($("src").value),r=this.getBoundingClientRect();
  pop(sev(it)==='ok'?'clean':sev(it),'Flow check',lintHTML(it,'py'),r.left-40,r.bottom+8)};
...
var _run2=run;
run=function(){_run2();
  paint($("icLint"),$("src").value.trim()?lintPy($("src").value):[],'Check the flow');
