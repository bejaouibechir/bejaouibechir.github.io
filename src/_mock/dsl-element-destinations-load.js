...
(function () {
  var DATA = {"cases":[{"value":"replace","label":"replace the target","yaml":"destinations:\n  output:\n    type: csv\n    connection: {base_path: out}\n    load:\n      table: result.csv\n      mode: replace\n      batch_size: 10000","notes":["the first batch replaces existing contents","later batches append within the same run","the verified job writes six rows"],"status":"✓ replace policy ready"},{"value":"append","label":"append this run","yaml":"destinations:\n  output:\n    type: csv\n    connection: {base_path: out}\n    load:\n      table: history.csv\n      mode: append","notes":["existing rows remain","every produced batch is appended","the default batch size is 10,000"],"status":"✓ append policy ready"},{"value":"small-batch","label":"replace in smaller batches","yaml":"destinations:\n  output:\n    type: csv\n    connection: {base_path: out}\n    load:\n      table: result.csv\n      mode: replace\n      batch_size: 100","notes":["the target and mode remain unchanged","the requested write batch is smaller","accepted values range from 10 to 100,000"],"status":"✓ batch size 100 accepted"}],"tour":[{"key":"root","title":"load","match":["load:"],"bubble":"The final write request.","context":"It belongs inside one destination declaration."},{"key":"target","title":"target","match":["table:","collection:"],"bubble":"The object that receives rows.","context":"Use table for files and tables, or collection for MongoDB."},{"key":"mode","title":"mode","match":["mode:"],"bubble":"How this run changes the target.","context":"Append is the default; replace clears first; upsert requires key columns and connector support."},{"key":"batch","title":"batch and key","match":["batch_size:","key:"],"bubble":"Controls for delivery and matching.","context":"Batch size is bounded; key is mandatory for an upsert declaration."}]};
